...
FROM province 
WHERE province.country in 
(SELECT country.code FROM country WHERE country.name = 'Sweden') 
...
WHERE organization.country in 
(SELECT encompasses.country FROM encompasses WHERE encompasses.continent='Europe') 
AND organization.name LIKE '%Nuclear%' AND organization.established IS NULL;
...
SELECT DISTINCT ON (continent) encompasses.continent,mountain.name,mountain.height 
FROM (mountain INNER JOIN geo_mountain ON mountain.name=geo_mountain.mountain 
			   INNER JOIN encompasses ON geo_mountain.country=encompasses.country) 
GROUP BY encompasses.continent,mountain.name,mountain.height 
ORDER by continent, height DESC;
...
SELECT name,SUM(borders.length)/COUNT(country2) as ratio 
FROM country INNER JOIN borders ON country.code=borders.country1 OR country.code=borders.country2 
GROUP BY country.name ORDER BY ratio DESC;
...
SELECT name,
	   (population*(POWER(1+population_growth/100,10))) as in10yrs,
       (population*(POWER(1+population_growth/100,25))) as in25yrs,
       (population*(POWER(1+population_growth/100,50))) as in50yrs,
       (population*(POWER(1+population_growth/100,100))) as in100yrs 
FROM country INNER JOIN population ON country.code=population.country 
GROUP BY name,population,population_growth 
ORDER BY name DESC;
...
AND ((borders.country1 IN (SELECT isMember.country FROM isMember WHERE organization='NATO' AND type='member') 
      OR borders.country2 IN (SELECT isMember.country FROM isMember WHERE organization='NATO' AND type='member')));
...
WITH RECURSIVE overlandCountries AS
(SELECT borders.country2,borders.country1 FROM borders WHERE borders.country1 in 
(SELECT country.code FROM country WHERE country.name = 'Mexico') OR borders.country2 in 
(SELECT country.code FROM country WHERE country.name = 'Mexico') 

UNION SELECT borders.country1, borders.country2 
FROM borders JOIN overlandCountries c 
ON(borders.country2=c.country1 OR borders.country1=c.country1 OR borders.country1=c.country2 OR borders.country2=c.country2))  
SELECT name FROM country WHERE code IN (SELECT country1 FROM overlandCountries UNION ALL SELECT country2 FROM overlandCountries 
GROUP BY overlandCountries.country2);
